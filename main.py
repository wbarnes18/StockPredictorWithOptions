...
def calculate_indicators(minute_data, hour_data, day_data):
    ...
    for data, granularity in [(minute_data, "minute"), (hour_data, "hour"), (day_data, "day")]:
        ...
        try:
            ...
            ind["VWAP"] = VolumeWeightedAveragePrice(data['High'], data['Low'], data['Close'],
                                                     data['Volume']).volume_weighted_average_price().iloc[-1]
            # OBV
...
def backtest_model(ticker, minute_data, hour_data, day_data, minute_pred, hour_pred, day_pred):
    ...
    minute_accuracy = 1 - abs(
        minute_pred - actual_minute) / actual_minute if actual_minute and minute_pred and actual_minute != 0 else 0
    hour_accuracy = 1 - abs(
        hour_pred - actual_hour) / actual_hour if actual_hour and hour_pred and actual_hour != 0 else 0
        ...
        if st.button("Predict"):
            try:
                ...
                if minute_data.empty and hour_data.empty and day_data.empty:
                    st.error(
                        "Failed to fetch stock data for all granularities. The market may be closed, or the ticker symbol may be invalid. Please try again during market hours (9:30 AM - 4:00 PM ET, Monday-Friday).")
                else:
                    ...
                    minute_acc, hour_acc, day_acc = backtest_model(ticker, minute_data, hour_data, day_data,
                                                                   minute_pred, hour_pred, day_pred)
    ...
    if any(
        ...
        try:
            ...
            if not (minute_data.empty and hour_data.empty and day_data.empty):
                ...
                st.warning(
                    "Self-improvement loop skipped: No data available for training (possibly due to market closure).")
        except Exception as e:
